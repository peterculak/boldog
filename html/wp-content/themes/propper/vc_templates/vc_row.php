...
if($type == '2'){
	$wrapper_attributes[] = 'style="color:' . esc_attr( $txt_clr ) . '"';
	...
	if ($bg_img != null) {	
		...
		$output .='<div class="bg-transfer opacity-15">
                    <img src="'.esc_url($bg_image_src).'" alt="img">
                </div>';
	} 
	...
	$output .='<div class="background-color '.$opacity.'" data-background-color-custom="'.$bg_clr.' "></div>
            </div></div>';
	$output .= $after_output;
	$output .= $this->endBlockComment( $this->getShortcode() );
